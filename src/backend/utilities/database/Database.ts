...
export default class Database {
    ...
    async save(tableName: string, repoObj: any) {
        ...
        for (let i = dbObj[1].length; i-- > 0;) {
            dbObj[1][i] = dbObj[1][i] === null ? 'NULL' : `${mysql.escape(dbObj[1][i])}`
        }

        const dbPrimaryObj = repoObj.getPrimaryDbPair();
        dbPrimaryObj[1] = dbPrimaryObj[1] === null ? 'NULL' : `${mysql.escape(dbPrimaryObj[1])}`

        let insert = false;
        if (repoObj.getPrimaryValue() === null) {
            insert = true;
        } else {
            const result = await this.query(`SELECT ${dbPrimaryObj[0]} FROM ${tableName} WHERE ${dbPrimaryObj[0]} = ${dbPrimaryObj[1]}`);
        ...
        if (insert === true) {
            ...
            const sqlResult = await this.query(`INSERT INTO ${tableName}(${columns}) VALUES(${values})`);

            if (repoObj.getPrimaryValueForInsert() === null) {
                repoObj.setPrimaryValue(sqlResult.insertId);
            }
        } else {
            const pairs = dbObj[0].map((_, i) => {
                return `${dbObj[0][i]} = ${dbObj[1][i]}`;
            }).join(',');

            await this.query(`UPDATE ${tableName} SET ${pairs} WHERE  ${dbPrimaryObj[0]} = ${dbPrimaryObj[1]}`)
        }
    ...
    async batchInsert(tableName: string, repoObjs: any[]) {
        ...
        for (let i = 0; i < repoObjs.length; ++i) {
            ...
            for (let j = dbObj[1].length; j-- > 0;) {
                dbObj[1][j] = dbObj[1][j] === null ? 'NULL' : `${mysql.escape(dbObj[1][j])}`;
            }
            ...
            for (let j = 0; j < values.length; j += 1000) {
                ...
                await this.query(`INSERT INFO ${tableName}(${columns}) VALUES(${slicedValuesAsString})`);
            }
        }
    }
    ...
    async fetch(tableName: string, columns: string[], databaseWhere: DatabaseWhere) {
        const result = [];
