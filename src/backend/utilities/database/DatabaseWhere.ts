...
export default class DatabaseWhere {
    ...
    build() {
        ...
        const sqlWhere = `${sqlColumns} ${sqlGroup} ${sqlOrder} ${sqlLimit} ${sqlOffset}`.trim();
    ...
    static makeRepoColumns(props: number[] | null, matcherCallback: (i: number) => string | null) {
        ...
        if (props === null) {
            ...
            if (columns.length === 0) {
                throw new Error('You must select at least one primitive property, because it is required in object construction');
            }
        }
