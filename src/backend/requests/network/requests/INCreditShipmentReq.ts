...
export default class INCreditShipmentReq {
    ...
    constructor(payload: Payload) {
        ...
        this.shipmentModel = ShipmentModel.fromNetwork(json.obj.shipmentModel);
        this.skuOriginModels = [];
        for (let i = 0; i < json.obj.skuOriginModels.length; ++i) {
            this.skuOriginModels.push(SkuOriginModel.fromNetwork(json.obj.skuOriginModels[i]));
        }
        this.skuModels = [];
        for (let i = 0; i < json.obj.skuModels.length; ++i) {
            this.skuModels.push(SkuModel.fromNetwork(json.obj.skuModels[i]));
        }
        this.shipmentDocumentModels = [];
        for (let i = 0; i < json.obj.shipmentDocumentModels.length; ++i) {
            this.shipmentDocumentModels.push(ShipmentDocumentModel.fromNetwork(json.obj.shipmentDocumentModels[i]));
        }
    }
